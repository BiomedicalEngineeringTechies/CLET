...
%% LED screen data
% User Inputs
DirPath = '28Jan2021_Pho_SC_WI_2'; % Directory path
FileName = '28Jan2021_Pho_SC_WI_2.eeg'; % File Name
nstim = 2; % Number of stimuli types
Lisi = 1; % Lower bound of interstimuli period (in s)
ThD1 = 60000; % Threshold1 for positive electrode
ThD2 = 20000; % Threshold2
...
PosPhoto = double(DataPhoto >= ThD1); 
PosDiff = [0 double(diff(PosPhoto))]; 
...
tsec_D1 = t(onsetsam_D1); % Time (in seconds s) of D1 event

% D2
NegPhoto = double(DataPhoto <= ThD2);
NegDiff = [0 double(diff(NegPhoto))];
...
tsec_D2 = t(onsetsam_D2); % Time (in seconds s) of D2 event
...
title('Photodiode signal with markers of stimuli & diode', 'fontweight', 'bold', 'FontSize', 24);
legend({'Photodiode signal', 'Trigger Sent S1', 'Trigger Sent S2', 'Trigger Detected D1', 'Trigger Detected D2'}, ...
...
LatD1S1 = LatD1S1 * 1000; % In milliseconds ms

aveLatD1S1 = mean(LatD1S1);
stdLatD1S1 = std(LatD1S1);  
S1D1col1 = [cellstr(num2str(tsec_S1, decpla)); ...
    'Not Applicable'];
S1D1col2 = [cellstr(num2str(tsec_D1, decpla)); ...
    'Not Applicable'];
S1D1col3 = [cellstr(num2str(LatD1S1, decpla)); ...
    cellstr(strcat(num2str(aveLatD1S1, decpla), char(177), num2str(stdLatD1S1, decpla)))];
varNames = {'Onset Time for Trigger S1 (in s)', ...
    'Onset Time for Diode D1 (in s)', ...
    'Latency between D1 and S1 (in ms)'};
T1 = table(S1D1col1, S1D1col2, S1D1col3, 'VariableNames', varNames);
writetable(T1, 'LEDScreenTableD1S1.xls', 'Sheet', 1);

% D2S2
LatD2S2 = tsec_D2 - tsec_S2; 
LatD2S2 = LatD2S2 * 1000; % In milliseconds ms

aveLatD2S2 = mean(LatD2S2); 
stdLatD2S2 = std(LatD2S2); 
S2D2col1 = [cellstr(num2str(tsec_S2, decpla)); ...
    'Not Applicable'];
S2D2col2 = [cellstr(num2str(tsec_D2, decpla)); ...
    'Not Applicable'];
S2D2col3 = [cellstr(num2str(LatD2S2, decpla)); ...
    cellstr(strcat(num2str(aveLatD2S2, decpla), char(177), num2str(stdLatD2S2, decpla)))];
varNames = {'Onset Time for Trigger S2 (in s)', ...
    'Onset Time for Diode D2 (in s)', ...
    'Latency between D2 and S2 (in ms)'};
T2 = table(S2D2col1, S2D2col2, S2D2col3, 'VariableNames', varNames);
writetable(T2, 'LEDScreenTableD2S2.xls', 'Sheet', 1);
